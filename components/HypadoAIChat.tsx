...
const HypadoAIChat: React.FC<Props> = ({ projects, clients, installers }) => {
    ...
    return (
        <div className={`fixed bottom-6 right-6 z-50 flex flex-col items-end pointer-events-none ${isOpen ? 'pointer-events-auto' : ''}`}>
            ...
            {/* CHAT WINDOW */}
            <div
                className={`bg-white w-[350px] md:w-[400px] h-[500px] rounded-[32px] shadow-2xl border border-slate-200 overflow-hidden flex flex-col transition-all duration-300 origin-bottom-right mb-4 pointer-events-auto ${isOpen ? 'scale-100 opacity-100' : 'scale-75 opacity-0 translate-y-10 invisible'}`}
            >
                {/* Header */}
                ...
                <form onSubmit={handleSend} className="p-4 bg-white border-t border-slate-100 flex gap-2">
                    <input
                        type="text"
                        className="flex-1 bg-slate-100 rounded-full px-5 py-3 text-sm font-medium outline-none focus:ring-2 focus:ring-indigo-500 transition-all placeholder:text-slate-400"
            ...
            <button
                ...
                className={`w-16 h-16 rounded-full shadow-2xl flex items-center justify-center transition-all duration-300 pointer-events-auto hover:scale-110 active:scale-95 ${isOpen ? 'bg-slate-200 text-slate-500 rotate-90' : 'bg-indigo-600 text-white rotate-0'}`}
            >
